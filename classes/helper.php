...
class tool_uploadpage_helper {
	...
	public static function get_or_create_category_from_import_record($record)
	{
		...
		if (!empty($record->course_categoryidnumber)) {
			$category_id = self::resolve_category_by_idnumber($record->course_categoryidnumber);
			if ($category_id === false) {
				...
				if (!empty($record->course_categoryname)) {
					...
					if (method_exists('\core_course_category', 'create')) {
						$created_category = core_course_category::create($category);
					} else {
						$created_category = coursecat::create($category);
					}
	 ...
	 *
	 * @param object $record Validated Imported Record
	 * @param string $pagetemplate The template to use to create the description, if not specified use the default self::$default_pagetemplate
	 * @return object course or null
	 */
